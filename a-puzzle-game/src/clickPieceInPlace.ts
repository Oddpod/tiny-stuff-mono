...
export function clickPieceIntoPlace({
	pieceId,
	left,
	top,
	boundingBox,
	definition,
	pieceSize,
}: ClickPieceIntoPlaceParams) {
	const boardBoundingBox = document
		.getElementById("board")!
		.getBoundingClientRect();
	const pieceDiv = document.getElementById(`piece-${pieceId}`);

	const isCornerPiece =
		Object.values(definition.sides).filter((sideType) => sideType === "flat")
			.length === 2;

	// TODO: refactor out to own method
	const shiftLeftBy =
		definition.sides.left === "ear" ? (15 * pieceSize) / PIECE_DIMENSIONS : 0;
	const shiftTopBy =
		definition.sides.top === "ear" ? (15 * pieceSize) / PIECE_DIMENSIONS : 0;

	const leftAdjustedForEars = boardBoundingBox.left - shiftLeftBy;
	const topAdjustedForEars = boardBoundingBox.top - shiftTopBy;

	const leftIsInsideBoundingBox = isWithinRangeInclusive(
		left,
		leftAdjustedForEars + boundingBox[0].x - HIT_OFFSET,
		leftAdjustedForEars + boundingBox[1].x - HIT_OFFSET,
	);
	const topIsInsideBoundingBox = isWithinRangeInclusive(
		top,
		topAdjustedForEars + boundingBox[0].y - HIT_OFFSET,
		topAdjustedForEars + boundingBox[1].y - HIT_OFFSET,
	);
	...
	if (canConnectPiece.toTheLeft) {
		...
		const {
			domRect,
			definition: wantedPieceDef,
			isOverLappingOnY,
		} = checkWantedPiece(wantedLeftPieceId, hitOffsetForEar);

		const isOverLappingOnX = isWithinRangeInclusive(
			pieceDomRect.left - domRect.right - HIT_OFFSET,
			-hitOffsetForEar,
			(-1 / 2) * hitOffsetForEar,
		);
		console.log({
			isOverLappingOnX,
			isOverLappingOnY,
			hmm: pieceDomRect.left - domRect.right - HIT_OFFSET,
			hitOffsetForEar,
		});
		...
		const {
			...
			definition: wantedPieceDef,
			isOverLappingOnY,
		} = checkWantedPiece(wantedRightPieceId, hitOffsetForEar);

		const isOverLappingOnX = isWithinRangeInclusive(
			pieceDomRect.right - domRect.left - HIT_OFFSET,
			-hitOffsetForEar,
			hitOffsetForEar,
		);
		console.log({
			isOverLappingOnX,
			isOverLappingOnY,
			hmm: pieceDomRect.right - domRect.left - HIT_OFFSET,
			hitOffsetForEar,
			case: "right",
		});
		if (isOverLappingOnX && isOverLappingOnY) {
		...
		const wantedRightPieceId = pieceId + 1;
		const { domRect, isOverLappingOnY } = checkWantedPiece(
			wantedRightPieceId,
			hitOffsetForEar,
		);
		...
		console.log({
			isOverLappingOnX,
			isOverLappingOnY,
			wanted: domRect.bottom,
			piece: pieceDomRect.bottom,
		});
		if (isOverLappingOnX && isOverLappingOnY) {
...
function checkWantedPiece(wantedLeftPieceId: number, hitOffsetForEar: number) {
	const wantedLeftPiece = document.getElementById(
		`piece-${wantedLeftPieceId}`,
	)! as HtmlPieceElement;
	const definition = pieceDefinitionLookup.get(
		Number.parseInt(wantedLeftPiece.dataset.definitionId),
	)!;
	const domRect = wantedLeftPiece.getBoundingClientRect();
	const isOverLappingOnY = isWithinRangeInclusive(
		domRect.top - domRect.top - HIT_OFFSET,
		(-1 / 2) * hitOffsetForEar,
		(1 / 2) * hitOffsetForEar,
	);
